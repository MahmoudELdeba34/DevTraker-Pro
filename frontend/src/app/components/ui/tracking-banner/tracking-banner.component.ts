    ...
    <div class="tracking-banner tracking-banner--compact flex flex-col md:flex-row md:items-center md:justify-between gap-3"
      ...
      <div class="flex flex-wrap items-center gap-3 md:gap-4 md:justify-end flex-1 min-w-0">
        @if (anyTimerRunning()) {
          ...
          <span class="text-sm font-bold text-white truncate min-w-0 max-w-[220px] md:max-w-sm">{{ runningLabel() }}</span>
          <span class="text-2xl md:text-3xl font-mono font-extrabold tabular-nums text-accent shrink-0">{{ displayTime() }}</span>
          <button
            type="button"
            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-danger text-white text-xs font-bold uppercase tracking-wider hover:bg-danger/90 transition-colors shrink-0 disabled:opacity-50"
            [disabled]="stopping()"
            (click)="stopAny()">
